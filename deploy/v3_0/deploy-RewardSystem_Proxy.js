// deploy: npx hardhat deploy --network base_goerli --tags RewardSystem_Proxy
...
module.exports = async function({deployments, getChainId, getNamedAccounts, getUnnamedAccounts}) {
	...
	// RewardSystem ERC1967 Proxy
	{
		...
		const impl_deployment = await deployments.get("RewardSystem");
		...
		const proxy_init_data = impl_contract.methods.postConstruct(ali_address || op_ali.address).encodeABI();

		// deploy ERC1967 proxy
		await deployments.deploy("RewardSystem_Proxy", {
			...
			args: [impl_deployment.address, proxy_init_data],
...
module.exports.tags = ["RewardSystem_Proxy", "v3_0_1", "deploy"];
module.exports.dependencies = ["RewardSystem"];
